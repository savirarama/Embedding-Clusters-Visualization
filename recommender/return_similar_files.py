...
def load_data_from_db(db_path, collection_name, repo_name):
    print(f"Connecting to ChromaDB at '{db_path}'.")
    chroma_client = chromadb.PersistentClient(path=db_path)
    collection = chroma_client.get_collection(name=collection_name)
    print(f"Connected to initial collection '{collection_name}'. Total items: {collection.count()}")
...
def get_recommended_files(query_hash, similar_commits, repo_path, remote_url, commit_hashes, metadatas, n=10, w=1.0):
    # Get files modified in query commit
    query_modified_files = json.loads(metadatas[commit_hashes.index(query_hash)]['modified_files'])
    # print("query modified files:")
    # print(query_modified_files)
    ...
    for commit in tqdm(similar_commits, desc="Processing similar commits to get files"):
        ...
        modified_files = json.loads(metadatas[commit_hashes.index(commit_hash)]['modified_files'])
...
if __name__ == "__main__":
    ...
    all_commit_hashes, all_commit_embeddings, all_metadatas = load_data_from_db(args.db_path, args.collection_name, args.repo_name)
    print(f"all commit hashes: {all_commit_hashes[:5]}")
    ...
    if all_commit_hashes is not None and all_commit_embeddings is not None and query_entries:
        ...
        # Sace commit recommendation results
        ...
        with open(save_commit_results_path, "w") as f:
            ...
            logging.info(f"\n✔️ Recommendation results written to {save_commit_results_path}")
